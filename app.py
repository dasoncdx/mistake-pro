...
async def login_page(request: Request):
    ...
    uhtml = ""
    for u in users:
        uhtml += f"""<div class="crd2"><form method="post" action="/login"><input type="hidden" name="name" value="{_esc(u['n'])}">
    ...
    if not users: uhtml = '<div style="text-align:center;color:var(--ts);font-size:14px;padding:40px 0;">还没有账号</div>'
    ...
    <div class="pg" style="padding:0 24px;">{f'<div class="err-msg">{_esc(err)}</div>' if err else ''}{uhtml}<div style="text-align:center;padding:20px;"><a href="/register" style="color:var(--b);font-size:14px;text-decoration:none;">＋ 创建新账号</a></div></div>"""
...
async def login_post(request: Request):
    ...
    ss(n); from db import init_db; init_db(n)
...
async def home(request: Request):
    ...
    if due:
        for k in due[:5]:
            ...
            dh += f"""<div style="margin-bottom:10px;"><div class="kpr"><span class="nm">{_esc(k['knowledge_point'])}</span><span class="pct" style="color:{color};">{int(sc*100)}%</span></div><div class="prog"><div class="pf" style="width:{int(sc*100)}%;background:{color};"></div></div></div>"""
        dh = f"""<div class="crd" style="border:1px solid rgba(91,127,255,.08);"><div style="font-size:15px;font-weight:600;color:var(--t);margin-bottom:12px;">📌 今日待复习</div>{dh}<a href="/mistake/new" class="btn btn-p">开始复习</a></div>"""
    ...
    <div style="font-size:13px;color:var(--ts);margin-bottom:20px;">{"今日有 <b>"+str(len(due))+"</b> 个知识点等待复习" if due else "今日无需复习 ✅"}</div>{dh}
    <div class="gr">
...
async def map_page(request: Request):
    ...
    for m in ms:
        ...
        bg = "var(--rb)" if sc<0.4 else "var(--w)"; pc = "#28A745" if sc>=0.9 else ("#3D5FD9" if sc>=0.7 else ("#D9821A" if sc>=0.4 else "#E04050"))
        ...
        mh += f"""<div class="crd2" style="background:{bg};"><div class="kpr"><span class="nm">{_esc(m['knowledge_point'])}</span><span class="pct" style="color:{color};">{int(sc*100)}%</span><span class="tag {tc}">{st}</span></div><div class="prog"><div class="pf" style="width:{int(sc*100)}%;background:{pc};"></div></div></div>"""
