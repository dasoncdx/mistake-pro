...
def _init():
    global _ready, _users
    if _ready: return
    import json, hashlib, random, string
    from dotenv import load_dotenv; load_dotenv()
    # demo 账号
    s = os.urandom(16); h1 = hashlib.pbkdf2_hmac("sha256", b"demo", s, 200000)
    s2 = os.urandom(16); h2 = hashlib.pbkdf2_hmac("sha256", b"test123", s2, 200000)
    _users = {
        "demo": {"student_name":"demo","password_hash":s.hex()+":"+h1.hex(),"province":"广东省","city":"广州市","district":"天河区","grade_level":"grade_4","curriculum_version":"人教版","subjects":["math"],"version":"basic"},
        "test": {"student_name":"test","password_hash":s2.hex()+":"+h2.hex(),"province":"广东省","city":"广州市","district":"天河区","grade_level":"grade_4","curriculum_version":"人教版","subjects":["math"],"version":"basic"},
    }
    # 尝试写DB（可写则写，不可写跳过）
    try:
        ...
        for name in ["demo","test"]:
            ...
            if not os.path.exists(pf): json.dump(_users[name],open(pf,"w"),ensure_ascii=False,indent=2)
            from db import init_db; init_db(name)
    except: pass
    _ready = True

_sessions = {}
def _sid(): return ''.join(__import__('random').choices(__import__('string').ascii_letters+__import__('string').digits, k=32))
...
async def login_page(request: Request):
    ...
    <div class="hd"><div style="font-size:28px;font-weight:700;color:var(--t);">错题<span style="color:var(--b);">Pro</span></div><div style="font-size:13px;color:var(--ts);">让每一道错题，变成知识版图上被征服的领地</div></div>
...
async def register_post(request: Request):
    ...
    try:
        ...
        __import__('json').dump(_users[n],open(os.path.join(d,"profile.json"),"w"),ensure_ascii=False,indent=2)
...
async def home(request: Request):
    ...
    return HTMLResponse(_html(body, "首页", ("1","","")))
...
function e(s){let d=document.createElement('div');d.textContent=s||'';return d.innerHTML}"""
    ...
    return HTMLResponse(_html(body, "录入错题"))
...
async def map_page(request: Request):
    ...
    if a: return a
    conn=request.state.conn
    if not conn: return HTMLResponse(_html('<div class="pg"><div class="nb"><a href="/home">← 返回</a><span class="tt">知识版图</span></div><div style="text-align:center;padding:60px;"><div style="font-size:40px;">🗺️</div><div>暂无数据</div></div></div>',"版图"))
    from db import get_all_masteries
    ms=get_all_masteries(conn,"math"); mh=""
    for m in ms:
    ...
    body=f'<div class="pg"><div class="nb"><a href="/home">← 返回</a><span class="tt">知识版图</span></div><div style="text-align:center;font-size:13px;color:var(--ts);margin-bottom:16px;">{mh or "<div>暂无数据</div>"}</div></div>'
    return HTMLResponse(_html(body,"版图",("","1","")))
...
async def report_page(request: Request):
    ...
    if not conn: return HTMLResponse(_html('<div class="pg"><div class="nb"><a href="/home">← 返回</a><span class="tt">学习报告</span></div><div style="text-align:center;padding:60px;">暂无数据</div></div>',"报告"))
    from db import get_all_masteries
    ms=get_all_masteries(conn,"math"); overall=sum(m["mastery_score"] for m in ms)/len(ms) if ms else 0; mh=""
    for m in ms:
    ...
    return HTMLResponse(_html(body,"报告",("","","1")))
...
def _html(body, title="错题Pro", nav=None):
    n=nav if nav else("","","")
